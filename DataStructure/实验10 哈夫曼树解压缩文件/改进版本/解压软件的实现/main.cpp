...
int main()
{
    ...
    while( !( infile.eof() ) ) {
        infile.read( &ch, sizeof(ch) );  //读取待解压文件内容
        string s = (*table)[ch];
    ...
    outfile.open( DECOMPRESS_FILE, ios::binary );
    char decode = '\0';
    ...
    while( !( infile.eof() ) ) {
        ...
        for( int i = 0; i < 8; i++ ) {
            ...
            if( x->isLeaf() ) {
                outfile.write( &(x->data.ch), sizeof(char) );
                x = tree->getRoot();
            }
        }
    }
